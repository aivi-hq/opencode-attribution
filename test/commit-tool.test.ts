...
async function committed(
  ...
  const tool = createCommitTool({
    version: "2.0.11",
    session: async () => ({ directory: session.directory, model: session.model }),
  })
  const context = {
    sessionID: "ses_test",
    agent: "build",
  ...
  const result = await tool.execute({ message, ...(args ? { args } : {}) }, context)
...
test("an attended commit gains the trailers; the person stays the author", async () => {
  ...
  assert.equal(git(dir, "log", "-1", "--pretty=%an <%ae>"), "Alice <alice@example.com>")
  assert.equal(git(dir, "log", "-1", "--pretty=%cn <%ce>"), "Alice <alice@example.com>")
})

test("an existing message body is preserved, trailers are appended last", async () => {
  const dir = await stagedRepo()
  await committed("Add a file\n\nBecause the plan says so.", { directory: dir, model })
  assert.deepEqual(
    git(dir, "log", "-1", "--pretty=%B").split("\n"),
    [
      "Add a file",
...
test("opencode.coauthor overrides the default co-author", async () => {
  const dir = await stagedRepo()
  const bot = "aivi-agent[bot] <331678708+aivi-agent[bot]@users.noreply.github.com>"
  git(dir, "config", "opencode.coauthor", bot)
  await committed("Add a file", { directory: dir, model })
  assert.ok(git(dir, "log", "-1", "--pretty=%B").includes(`Co-authored-by: ${bot}`))
})
...
test("an autonomous worktree commits untouched, bot author intact", async () => {
  ...
  git(dir, "config", "user.email", "331678708+aivi-agent[bot]@users.noreply.github.com")
  await committed("Add a file", { directory: dir, model })

  assert.deepEqual(git(dir, "log", "-1", "--pretty=%B").split("\n"), ["Add a file"])
  assert.equal(git(dir, "log", "-1", "--pretty=%an <%ae>"), "aivi-agent[bot] <331678708+aivi-agent[bot]@users.noreply.github.com>")
})
...
test("extra git arguments pass through", async () => {
  const dir = await stagedRepo()
  await committed("Add a file", { directory: dir, model })
  await committed("Add a file, amended", { directory: dir, model }, ["--amend", "--no-edit"])
  assert.deepEqual(
    git(dir, "log", "-1", "--pretty=%B").split("\n"),
    [
      "Add a file, amended",
