...
import { DEFAULT_COAUTHOR, harness, isAutonomous, trailers } from "../src/attribution.ts"
...
test("an attended commit gets the co-author and the harness line, in order", () => {
  assert.deepEqual(
    ...
    [
      ...
      { token: "Harness", value: "OpenCode v2.0.11, mlx-serve/Qwen3.8-Flash-Next" },
    ],
  )
})
...
test("a configured co-author is used verbatim", () => {
  ...
  const [first] = trailers({ autonomous: false, coauthor: bot, version: "2.0.11" })
...
test("the harness line normalizes the version and survives a missing model", () => {
  ...
  assert.equal(harness("2.1.0", { id: "m", providerID: "p" }), "OpenCode v2.1.0, p/m")
