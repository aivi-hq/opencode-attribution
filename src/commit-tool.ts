...
const input = {
  ...
  properties: {
    ...
    args: {
      type: "array",
      items: { type: "string" },
      description: 'Extra git commit arguments, e.g. ["-a"] or ["--amend", "--no-edit"].',
    },
  },
  required: ["message"],
...
export function createCommitTool(deps: CommitToolDeps): ToolInfo<typeof input> {
  return {
    ...
    execute: async (parameters, context) => {
      ...
      const coauthor = (await configGet("opencode.coauthor", cwd)) || DEFAULT_COAUTHOR

      const trailerArgs: string[] = []
      for (const trailer of trailers({ autonomous, coauthor, version: deps.version, model: session.model })) {
        trailerArgs.push("--trailer", `${trailer.token}: ${trailer.value}`)
      }

      const result = await run(["commit", "-m", message, ...trailerArgs, ...(args ?? [])], cwd)
      const output = [result.stdout, result.stderr].map((part) => part.trim()).filter(Boolean).join("\n")
      if (result.code !== 0) {
        throw new Error(output || `git commit failed (exit code ${result.code})`)
      }
